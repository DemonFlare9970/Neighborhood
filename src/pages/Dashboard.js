...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const bal = txs.reduce((sum, tx) => tx.type === 'income' ? sum + tx.amount : sum - tx.amount, 0);
    setBalance(bal);
    setRecent(txs.slice(-5).reverse());
    setIncome(txs.filter(tx => tx.type === 'income').reduce((sum, tx) => sum + tx.amount, 0));
    setExpenses(txs.filter(tx => tx.type === 'expense').reduce((sum, tx) => sum + tx.amount, 0));
    // Budget
